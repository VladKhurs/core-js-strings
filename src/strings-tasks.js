...
function removeFirstOccurrences(str, value) {
  const firstI = str.indexOf(value);
  if (firstI === -1) {
    return str;
  }
  return str.slice(0, firstI) + str.slice(firstI + value.length);
}
...
function removeLastOccurrences(str, value) {
  ...
  return str.slice(0, lastI) + str.slice(lastI + value.length);
}
...
function reverseWords(str) {
  let s = str.split(' ');
  s = s.map((e) => e.split('').reverse().join(''));
  return s.join(' ');
}
